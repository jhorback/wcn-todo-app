...
@customDataElement('wcn-todo-de-data', {
    eventsListenAt: "parent"
})
class TodoDeData extends DataElement {
    state = TodoDataDef.defaultState;

    // tying the stateId property to the state-id attribute
    get stateId():string { return this.getAttribute("state-id") || ""; }
    set stateId(stateId:string) { this.setAttribute("state-id", stateId); }

    static get observedAttributes() { return ["state-id"] }

    attributeChangedCallback(name:string, oldValue:string, newValue:string) {

        if (name === "state-id") { 
            // the state-id is changing so we need to
